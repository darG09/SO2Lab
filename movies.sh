...
function query_year () {
    # Returns list of movies from ${1} produced later than ${2}
    local -r MOVIES_LIST=${1}
    local -r QUERY=${2}
    ...
    for MOVIE_FILE in ${MOVIES_LIST}; do
        local MOVIE_YEAR
        MOVIE_YEAR=$(grep "| Year" "${MOVIE_FILE}" | cut -d '|' -f2 | tr -d '[:space:]')
        if [[ "${MOVIE_YEAR}" -gt "${QUERY_YEAR}" ]]; then
            RESULTS_LIST+=( "${MOVIE_FILE}" )
        fi
    done
    echo "${RESULTS_LIST[@]:-}"
...
while getopts ":hd:t:a:f:x:y:" OPT; do
    case ${OPT} in
        ...
        t)
            ...
            ;;
        f)
            FILE_4_SAVING_RESULTS=${OPTARG:-results.txt}
            ;;
        a)
...
if [[ -z "${MOVIES_DIR:-}" ]]; then
    print_error "ERROR: Option -d is required"
    exit 1
fi
